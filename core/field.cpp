...
{
    __m128i mask = this->data[0].data;

    for (u8 cell = 1; cell < cell::COUNT; ++cell) {
        mask |= this->data[cell].data;
    }

    return FieldBit(mask);
};
...
u8 Field::get_drop_pair_frame(i8 x, direction::Type direction, const u8 heights[6])
{
    return 1 + (heights[x] != heights[x + direction::get_offset_x(direction)]);
};
...
void Field::drop_puyo(i8 x, cell::Type cell)
{
    ...
    this->drop_puyo(x, height, cell);
};

void Field::drop_puyo(i8 x, u8 height, cell::Type cell)
{
    assert(x >= 0 && x < 6);
...
void Field::drop_pair(i8 x, direction::Type direction, cell::Pair pair, const u8 heights[6])
{
    assert(x >= 0 && x < 6);
    ...
    {
    case direction::Type::UP:
        this->drop_puyo(x, heights[x], pair.first);
        this->drop_puyo(x, heights[x] < 13 ? heights[x] + 1 : heights[x], pair.second);
        break;
    case direction::Type::RIGHT:
        this->drop_puyo(x, heights[x], pair.first);
        this->drop_puyo(x + 1, heights[x + 1], pair.second);
        break;
    case direction::Type::DOWN:
        this->drop_puyo(x, heights[x], pair.second);
        this->drop_puyo(x, heights[x] < 13 ? heights[x] + 1 : heights[x], pair.first);
        break;
    case direction::Type::LEFT:
        this->drop_puyo(x, heights[x], pair.first);
        this->drop_puyo(x - 1, heights[x - 1], pair.second);
