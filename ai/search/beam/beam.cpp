...
)
{
    ...
    // Generates moves
    auto locks = move::generate(node.field, pair.first == pair.second, heights);
    ...
    for (auto i = 0; i < locks.get_size(); ++i) {
        ...
        // Drops pair
        child.field.drop_pair(locks[i].x, locks[i].r, pair, heights);
        ...
        // Evaluates action
        i32 tear = node.field.get_drop_pair_frame(locks[i].x, locks[i].r, heights) - 1;
...
{
    expand_impl(pair, node, w, callback);
};
...
)
{
    ...
    for (auto& node : parents.data) {
        expand_impl(pair, node, w, [&] (node::Data& child, const move::Placement& placement, const chain::Score& chain) {
