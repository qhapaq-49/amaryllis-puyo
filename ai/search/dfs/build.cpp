...
Result search(Field field, cell::Queue queue, eval::Weight w, i32 thread_count)
{
    ...
    u8 root_heights[6];
    field.get_heights(root_heights);
    auto placements = move::generate(field, queue[0].first == queue[0].second, root_heights);
    ...
    for (i32 t = 0; t < thread_count; ++t) {
        threads.emplace_back([&] () {
            while (true)
            {
                ...
                child.field.drop_pair(placement.x, placement.r, queue[0], root_heights);
                ...
                child.tear += root.field.get_drop_pair_frame(placement.x, placement.r, root_heights) - 1;
...
eval::Result dfs(Node& node, cell::Queue& queue, eval::Weight& w, i32 depth)
{
    ...
    auto placements = move::generate(node.field, queue[depth].first == queue[depth].second, heights);

    for (i32 i = 0; i < placements.get_size(); ++i) {
        // Creates child node
        auto child = node;
        child.field.drop_pair(placements[i].x, placements[i].r, queue[depth], heights);
        auto mask_pop = child.field.pop();
        ...
        // Updates stats
        child.tear += node.field.get_drop_pair_frame(placements[i].x, placements[i].r, heights) - 1;
        child.waste += mask_pop.get_size();
